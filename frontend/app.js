      ...
      msg.querySelectorAll('.euler-perm-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (approved) {
            ...
            // Decline — orchestrator removed, just acknowledge
...
// _handleEulerEvent removed — dead orchestrator code
...
function _eulerResetToIdle() {
  // Orchestrator removed — back button just resets local state
  _eulerBusy = false;
  _eulerStarted = false;
  const input = document.getElementById('euler-input');
...
// bdSnapshotCurrentScene_LEGACY removed
