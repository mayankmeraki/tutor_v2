...
from app.services.beat_parser import StreamingBeatDetector
from app.services.tts_service import elevenlabs_tts
...
class SessionRouter:
    ...
    async def _run_turn(
        ...
        try:
            from app.api.routes.chat import _generate_for_turn
            ...
            if context and isinstance(context, dict):
                teaching_mode = context.get("teachingMode", "voice")
            is_voice = teaching_mode == "voice" and self.voice_enabled

            beat_detector = StreamingBeatDetector() if is_voice else None
            ...
            async for sse_str in _generate_for_turn(
                ...
                # ── Voice mode: intercept text deltas for beat detection ──
                if is_voice and evt_type == "TEXT_MESSAGE_CONTENT":
