...
def build_triage_prompt(context_data: dict) -> str:
    """Build the full triage system prompt with student context + UI contract."""
    from app.agents.prompts.tags import TAGS_PROMPT
    from app.agents.prompts.toolkit import DELEGATE_TOOLKIT_PROMPT

    parts = [TRIAGE_SYSTEM_PROMPT, DELEGATE_TOOLKIT_PROMPT, TAGS_PROMPT]
    ...
    if student_model:
        ...
        parts.append("\n[STUDENT MODEL]\nNew student — no prior data. Need full diagnostic.\n")

    # Session history
    session_history = context_data.get("sessionHistory")
    if session_history:
        parts.append(f"\n[LAST SESSIONS]\n{session_history}\n")
    else:
    ...
    # Intent + scenario
    intent = context_data.get("intent") or context_data.get("studentIntent") or ""
    scenario = context_data.get("scenario") or "free"
    parts.append(f"\n[INTENT]\n{intent}\n")
    parts.append(f"[SCENARIO]\n{scenario}\n")
    # Hint for the agent based on scenario
    if intent.startswith("Teach me:"):
        parts.append("(Student clicked a specific lesson — focus on readiness diagnosis, not intent clarification)\n")
    elif scenario == "video_follow":
        parts.append("(Video follow-along mode — check prerequisites for the lecture content)\n")
    elif len(intent.split()) < 4:
        parts.append("(Vague intent — need both intent clarification AND level diagnosis)\n")

    # Knowledge summary
    knowledge = context_data.get("knowledgeSummary")
    if knowledge:
        parts.append(f"\n[STUDENT KNOWLEDGE STATE]\n{knowledge}\n")

    # Available content (from content_search, if pre-fetched)
    available = context_data.get("availableContent")
    if available:
        parts.append(f"\n[AVAILABLE CONTENT]\n{available}\n")

    # Upcoming topics (between-chunk triage — probe for THESE)
    upcoming = context_data.get("upcomingTopics")
    if upcoming:
        parts.append(f"\n[UPCOMING TOPICS — probe readiness for these]\n{upcoming}\n")

    # Last assessment results (between-chunk triage)
    last_assessment = context_data.get("lastAssessment")
    if last_assessment:
        parts.append(f"\n[LAST ASSESSMENT RESULTS]\n{last_assessment}\n")
        parts.append(
            "(Assessment just happened — these results are fresh signal. "
            "You may only need 0-1 questions to confirm readiness for next topics. "
            "If assessment clearly shows the student is ready, just say so and return.)\n"
        )
